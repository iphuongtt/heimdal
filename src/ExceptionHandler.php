...
class ExceptionHandler extends LaravelExceptionHandler
{
    ...
    public function __construct(Container $container)
    {
        ...
        $this->config = $container['config']->get('iphuongtt.heimdal');
    ...
    public function render($request, Exception $e)
    {
        ...
        if ($this->config['add_cors_headers']) {
            if (!class_exists(CorsService::class)) {
                throw new InvalidArgumentException(
                    'asm89/stack-cors has not been installed. Iphuongtt\Heimdal needs it for adding CORS headers to response.'
                );
            }
